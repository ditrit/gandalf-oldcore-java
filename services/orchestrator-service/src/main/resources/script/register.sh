...
svc_def=artifact-service.service.gandalf:10000/download/${svc_name}_${svc_version}.ini
...
svc_tar=artifact-service.service.gandalf:10000/download/$artifact_tar
tmp_tar=/tmp/$artifact_tar
wget -O $tmp_tar $svc_tar

echo "#!/bin/bash" > $tmp_sh
cat $tmp_vars >> $tmp_sh
cat /root/script/service.script >> $tmp_sh
chmod a+x $tmp_sh
