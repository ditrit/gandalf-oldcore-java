...
public class DeployJob implements JobHandler {
    ...
    @Autowired
    public DeployJob(ZeebeClient zeebe, DeployFeign deployFeign, ClientGandalf clientGandalf, ThreadPoolTaskExecutor threadPoolTaskExecutor, DeployJobProperties deployJobProperties) {
        this.zeebe = zeebe;
    ...
    public void handle(JobClient jobClient, ActivatedJob activatedJob) {
        ...
        ZMsg resultCommand = this.clientGandalf.getClientCommand().sendCommandSync("deploy", this.deployJobProperties.getConnectorEndPointName(), "WORKER_SERVICE_CLASS_STANDARD", "DEPLOY", "5", payload.toString());
        ...
        if(succes) {
            ...
            this.clientGandalf.getClientEvent().sendEvent("build", "DEPLOY", "5",projectName + " deploy : success" );
            jobClient.newCompleteCommand(activatedJob.getKey()).variables(workflow_variables).send().join();
        }
        else {
            this.clientGandalf.getClientEvent().sendEvent("build", "DEPLOY", "5",projectName + " deploy : fail" );
