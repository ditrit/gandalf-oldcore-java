...
public class ConnectorVersionControlManager {
    ...
    public CustomMergeRequest parseEventMergeRequest(String mergeRequestBody) throws IOException {
        ...
        CustomMergeRequest customMergeRequest = new CustomMergeRequest(rootNode.path("project").path("name").textValue(), rootNode.path("project").path("git_http_url").textValue(), rootNode.path("object_attributes").path("source_branch").textValue(), rootNode.path("object_attributes").path("target_branch").textValue());
